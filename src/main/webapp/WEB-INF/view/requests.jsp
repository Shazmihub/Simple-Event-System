		...
		function getEventRequests() {
            $.ajax({
                ...
                success: function (result) {
                    ...
                    $("#requestList").empty();
                    var receptionistData = "<table class='table table-hover'><thead><tr><th scope='col'>#</th><th scope='col'>Event Name</th><th scope='col'>First Name</th><th scope='col'>Last Name</th><th scope='col'>Current Status</th><th scope='col'>Requested on</th><th scope='col' style='text-align: center'>ACTION</th></tr></thead><tbody>";
                    var count = 1;
                    for (var i in result) {
                        receptionistData = receptionistData + "<tr><th scope='row'>" + count + "</th><td>" + result[i].event.name + "</td><td>" + result[i].requestedBy.firstName + "</td><td>" + result[i].requestedBy.lastName + "</td><td>" + result[i].status + "</td><td> " + moment(result[i].requestedDate).format("DD/MM/YYYY") + "</td><td style='text-align: center'><button  id='updateStatus" + i + "' onclick='approveRequest(" + result[i].id + ");' class='btn btn-primary'><i class='fa fa-check fa-fw'></i>APPROVE</button> <button id='deleteReceptionist" + i + "' onclick='declineRequest(\"" + result[i].id + "\");' class='btn btn-danger' ><i class='fa fa-times fa-fw'></i>DECLINE</button></td></tr>";
                        count = count + 1;
                    }
                    if (count === 1) {
