...
<head>
	...
	var authUserUrl = "${authUserUrl}";		
		...
		function getEventRequests() {
            $.ajax({
                type: "GET",
                url: "http://localhost:8080/api/requests/"+localStorage.getItem("userId"),
                contentType: 'application/json',
                success: function (result) {


                    $("#requestList").empty();
                    var receptionistData = "<table class='table table-hover'><thead><tr><th scope='col'>#</th><th scope='col'>Event Name</th><th scope='col'>Requested on</th><th scope='col'>Action done by</th><th scope='col' >Action done on</th><th scope='col'>Current Status</th></tr></thead><tbody>";
                    var count = 1;
                    for (var i in result) {
                    	if(result[i].actionBy === null){
                    		receptionistData = receptionistData + "<tr><th scope='row'>" + count + "</th><td>" + result[i].event.name + "</td><td> " + moment(result[i].requestedDate).format("DD/MM/YYYY") + "</td><td>-</td><td>-</td><td><button class='btn btn-warning'>" + result[i].status + "</button></td></tr>";
                    	} else {
                    		
                    		if(result[i].status==="APPROVED"){
                    			receptionistData = receptionistData + "<tr><th scope='row'>" + count + "</th><td>" + result[i].event.name + "</td><td> " + moment(result[i].requestedDate).format("DD/MM/YYYY") + "</td><td>" + result[i].actionBy.firstName +" "+ result[i].actionBy.lastName + "</td><td>" + moment(result[i].actionDate).format("DD/MM/YYYY") + "</td><td><button class='btn btn-success'>" + result[i].status + "</button></td></tr>";
                    		} else {
                    			receptionistData = receptionistData + "<tr><th scope='row'>" + count + "</th><td>" + result[i].event.name + "</td><td> " + moment(result[i].requestedDate).format("DD/MM/YYYY") + "</td><td>" + result[i].actionBy.firstName +" "+ result[i].actionBy.lastName + "</td><td>" + moment(result[i].actionDate).format("DD/MM/YYYY") + "</td><td><button class='btn btn-danger'>" + result[i].status + "</button></td></tr>";
                    		}
